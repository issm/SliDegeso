class @SliDegeso
    ...
    constructor: ->
        ...
        @geso_rate =
            ...
            m: 1920 / 1920
        @size = 'm'
        @wiper_enabled = @config.wiper_enabled ? true
        @help_showed = false

        @__data

        # URL変化時のハンドラ
        @handle_url_change = (url, url_before) =>
            m = url.match /\#!(.*)$/
            #h = m[1] ? ''
    ...
    _toggle_wiper_status: () ->
        @wiper_enabled = not @wiper_enabled
        @_update_wiper_status()


    _update_wiper_status: () ->
        $navi = @__data.current_slide.find '.wiper-status'
        if @wiper_enabled
    ...
    _show_help: () ->
        ...
        if @help_showed then $help.hide() else $help.show()
        @help_showed = not @help_showed
    ...
    # URL変化を感知する
    #
    #   1. @handle_url_change(url, url_before) を定義する
    #   2. listen_url_change() する
    #
    _listen_url_change: (interval) ->
        @__urlchev__url_before = location.href
        @__urlchev__timer = setInterval () =>
            if location.href isnt @__urlchev__url_before
                ( @handle_url_change ? () -> )( location.href, @__urlchev__url_before )
                @__urlchev__url_before = location.href
        , interval ? 50
...
#
$ ->
    window._slidegeso = new SliDegeso
